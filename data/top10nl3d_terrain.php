...
$conn = pg_pconnect("host=192.168.24.15 dbname=research user=geodan password=Gehijm");
if (!$conn) {
...
pieces AS (
		SELECT ST_Reverse(wkb_geometry) geom, ogc_fid id, type_landgebruik type_landg 
		FROM top103d.terreinvlak_3d_lod0 a, bounds b
		WHERE ST_Intersects(a.wkb_geometry, b.geom)
		UNION
		SELECT ST_Reverse(wkb_geometry) geom, ogc_fid id, 'wegvlak'::text type_landg 
		FROM top103d.wegdeelvlak_3d_lod0 a, bounds b
		WHERE ST_Intersects(a.wkb_geometry, b.geom)
		UNION
		SELECT ST_Reverse(wkb_geometry) geom, ogc_fid id, 'water'::text type_landg 
		FROM top103d.waterdeelvlak_3d_lod0 a, bounds b
		WHERE ST_Intersects(a.wkb_geometry, b.geom)
)
